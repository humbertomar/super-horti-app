...
export class CartPage {
    ...
    constructor(
        ...
        } else if (diaPedido == 2 && horaPedido > this.horario_maximo_pedidos) {
            //terça depois das 20:00
            this.data_entrega = moment().add(2, 'day').format('YYYY-MM-DD');
            this.dia_semana = moment().add(2, 'day').format('dddd'); //entrega quinta
        } else if (
            diaPedido == 3 &&
        ...
        } else if (diaPedido == 3 && horaPedido > this.horario_maximo_pedidos) {
            //quarta depois das 20:00
            this.data_entrega = moment().add(2, 'day').format('YYYY-MM-DD');
        ...
        } else if (diaPedido == 4 && horaPedido > this.horario_maximo_pedidos) {
            //quinta depois das 20:00
        ...
        } else if (diaPedido == 5 && horaPedido > this.horario_maximo_pedidos) {
            //sexta depois das 20:00
            this.data_entrega = moment().add(3, 'day').format('YYYY-MM-DD');
            this.dia_semana = moment().add(3, 'day').format('dddd'); //entrega segunda
        } else if (
            diaPedido == 6 &&
        ...
        } else if (diaPedido == 6 && horaPedido > this.horario_maximo_pedidos) {
            //sábado depois das 20:00
            this.data_entrega = moment().add(2, 'day').format('YYYY-MM-DD');
            this.dia_semana = moment().add(2, 'day').format('dddd'); //entrega segunda
        }
        ...
        if (localStorage.getItem('uid')) {
            if (!localStorage.getItem('keyAddress')) {
                //corrige um bug devido a essa nova variavel nao existir nas versoes antigas
  ...
  ionViewDidLoad() {
    ...
    localStorage.removeItem('logradouro');//limpa o endereco do localStorage*/
        ...
        if (localStorage.getItem('uid')) {
            ...
            this.authService.getData('settings').then(
                (data: any) => {
                    this.settings = data;
                    ...
                    this.settings.forEach((elemento) => {
                        ...
                        this.horario_maximo_pedidos =
                            elemento.horario_maximo_pedidos;
                ...
                (data: any[]) => {
                    ...
                    if (this.version != this.settings[0].versao) {
                        let alert = this.alertCtrl.create({
                            title: 'Atenção!',
                            subTitle:
                                'Você precisar atualizar o aplicativo do Super Horti!',
                            buttons: [
                                {
                                    text: 'OK',
                                    role: 'cancel',
    ...
    addressUser(event) {
        ...
        if (localStorage.getItem('uid')) {
            const subscribe = this.fbs
                .get(
                    '/users/' + this.af.auth.currentUser.uid + '/address/',
                    event
                )
                .subscribe((c: any) => {
    ...
    checarValorProdutos() {
        ...
        for (let i = 0; i <= this.Cart.length - 1; i++) {
            ...
            this.authService
                .getData('productID/' + this.Cart[i].item.itemId)
                .then(
                    (data: any[]) => {
                        this.settings = data;
                        ...
                        if (
                            Number(this.settings[0].price).toFixed(2) !=
                            Number(this.Cart[i].item.price).toFixed(2)
                        ) {
          ...
          this.Cart[i].item.price = (this.settings[0].price  * 2);*/
                            ...
                            this.Cart[i].itemTotalPrice =
                                this.settings[0].price *
                                this.Cart[i].item.itemQunatity;
                            ...
                            if (this.Cart.length - 1 == i) {
                                let alert = this.alertCtrl.create({
                                    title: 'Atenção',
                                    subTitle:
                                        'O(s) produto(s) que estão no seu carrinho sofreram alteração de preço. Os valores foram ajustados automaticamente para o preço atual!',
                                    buttons: ['OK'],
                                });
                                alert.present();
                            }
    ...
    getDataGoogleMaps(cep, bairro, endereco) {
        if (cep != 1) {
            this.authService.getData('googleMaps/' + cep).then(
                (result) => {
                    ...
                    if (
                        ...
                        localStorage.setItem(
                            'frete',
                            JSON.stringify(this.dataSetGoogleMaps)
                        );
    ...
    console.log("Dia da semana da entrega do pedido agendado: "+this.diaSemanaAgendado);*/
        ...
        if (this.totalItems < 10) {
                ...
                if (this.pedidoAgendado != '1') {
                    ...
                    if (dataEntrega[2] == 2 && dataEntrega[1] == 4) {
                        ...
                        let alert = this.alertCtrl.create({
                            title: 'Atenção!',
                            subTitle:
                                'Devido ao Réveillon nossa agenda está totalmente preenchida para entregas na quinta-feira (31/12). Por favor, escolha outra data!',
                            buttons: [
                                {
                                    text: 'OK',
                                    role: 'cancel',
            ...
            alert.present();*/
                        ...
                        if (this.totalItems < 10) {
                            this.showAlert(
                                'É necessário um pedido mínimo de 10 itens!'
                            );
                        } else {
                            let alert = this.alertCtrl.create({
                    ...
                    if (this.totalItems < 10) {
                        ...
                        let alert = this.alertCtrl.create({
                            ...
                            subTitle:
                                ...
                                dataEntrega[0] +
                                ' - ' +
                                this.diaSemanaAgendado.toUpperCase(),
                            buttons: [
                                {
                                    text: 'Sim',
                                    handler: (data) => {
        ...
        if (data.item.itemQunatity < 20) {
            ...
            for (let i = 0; i <= this.Cart.length - 1; i++) {
                ...
                if (
                    ...
                    for (
                        let j = 0;
                        j <= this.Cart[i].item.extraOptions.length - 1;
                        j++
                    ) {
                        ExtotalPrice =
                            ExtotalPrice +
                            this.Cart[i].item.extraOptions[j].value;
                    }
                    if (this.Cart[i].item.price.specialPrice) {
                        totalPrice =
                            ExtotalPrice + this.Cart[i].item.price.specialPrice;
                    } else {
                        totalPrice = ExtotalPrice + this.Cart[i].item.price;
                    }
                    this.Cart[i].itemTotalPrice =
                        totalPrice * data.item.itemQunatity;
                }
            }
